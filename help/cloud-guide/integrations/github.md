...
在GitHub开发人员设置中创建经典个人访问令牌。 您必须是具有对GitHub存储库的写入权限的组的成员，以便您可以 _推送_ 到存储库。 创建令牌时包括以下范围：
...
从现有环境克隆Adobe Commerce on cloud infrastructure项目，并将项目分支迁移到空的新的GitHub存储库，并保留相同的分支名称。 它是 **关键** 以保留相同的Git树，从而不会丢失Adobe Commerce on cloud基础架构项目中的任何现有环境或分支。
...
1. 将项目文件推送到新的GitHub存储库。 请记住保持所有分支名称相同。
   ...
   如果您从新的GitHub存储库开始，则可能必须使用 `-f` 选项，因为远程存储库与您的本地副本不匹配。

1. 验证GitHub存储库是否包含所有项目文件。

## 启用GitHub集成

在开始之前，您的项目代码和环境必须位于GitHub存储库中。 启用集成后，GitHub存储库将成为代码源。 如果您将代码更改推送到原始的 `magento` ，则在将代码更改推送到GitHub存储库时，集成会覆盖该内容。

以下内容启用GitHub集成，并提供在创建webhook时使用的有效负载URL。

>[!WARNING]
>
>以下命令将覆盖 _所有_ 使用GitHub存储库中的代码构建Adobe Commerce on cloud infrastructure项目中的代码，其中包括所有分支，包括 `production` 分支。 此操作立即发生且无法撤消。 最佳做法是，在云基础架构项目中从Adobe Commerce克隆所有分支，并将它们推送到GitHub存储库，这一点很重要 **早于** 添加GitHub集成。

您可以选择使用逐步完成CLI提示 `magento-cloud integration:add` 或者，您可以使用以下选项构建集成命令：
...
| `--build-pull-requests` | 可选 | Adobe Commerce指示合并拉取请求(`true` 默认) |
| `--fetch-branches` | 可选 | 促使云基础架构上的Adobe Commerce跟踪分支并在您更新分支后部署(`true` 默认) |
| `--prune-branches` | 可选 | 删除远程上不存在的分支(`true` 默认) |
...
1. 在 _Webhooks_ 窗格，单击 **添加webhook**.

1. 在 _Webhook/添加webhook_ 表单中，编辑以下字段：

   - **负载URL**：输入启用GitHub集成时返回的URL。
   - **内容类型**：选择 **application/json** 从名单上。
   - **密码**：输入验证密码。
   - **您希望触发此webhook的哪些事件？**：选择 **发送所有内容给我**.
   - 选择 **活动** 复选框。
...
配置GitHub集成后，您可以使用验证集成是否可以正常运行 `magento-cloud` CLI：
...
此外，您还可以通过登录到GitHub帐户并删除中的Web挂接来删除GitHub集成 _Webhooks_ 存储库的选项卡 _设置_.
