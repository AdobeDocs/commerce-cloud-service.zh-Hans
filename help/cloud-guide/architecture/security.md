...
所有AWS活动都登录到AWS CloudTrail。 操作系统、应用程序服务器和数据库日志存储在生产服务器上，并存储在备份中。 所有源代码更改都记录在Git存储库中。 部署历史记录在Adobe Commerce [Project Web界面](../project/overview.md)中可用。 所有支持访问都将被记录，支持会话也被记录。
