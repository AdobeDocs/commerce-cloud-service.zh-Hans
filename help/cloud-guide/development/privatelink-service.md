...
云基础架构上的Adobe Commerce支持与 [AWS PrivateLink](https://aws.amazon.com/privatelink/) 或 [Azure专用链接](https://learn.microsoft.com/en-us/azure/private-link/) 服务。 您可以使用PrivateLink在云基础架构环境上的Adobe Commerce与托管在外部系统上的服务和应用程序之间建立安全的私有通信。 Adobe Commerce应用程序和外部系统都必须可以通过在同一云区域内的同一云平台(AWS或Azure)上配置的虚拟专用云(VPC)端点访问。

>[!TIP]
>
>PrivateLink最适合用于保护非HTTP集成（如数据库或文件传输）的连接。 如果您计划将应用程序与Adobe Commerce API集成，请参阅如何创建 [AdobeAPI网格](https://developer.adobe.com/graphql-mesh-gateway/gateway/create-mesh/) 在 _适用于Adobe Developer App Builder的API网格_.
...
- 客户负责与管理自己的VPC相关的成本。
- 由于以下原因，您无法使用HTTPS协议（端口443）通过Azure专用链接连接到云基础架构上的Adobe Commerce [Fastly起源遮蔽](https://experienceleague.adobe.com/docs/commerce-knowledge-base/kb/faq/fastly-origin-cloaking-enablement-faq.html). 此限制不适用于AWS PrivateLink。
- PrivateDNS不可用。
...
- **单向PrivateLink** — 选择此配置可从Adobe Commerce上的云基础架构存储安全地检索数据。
- **双向专用链接** — 选择此配置以在云基础架构环境上建立与Adobe Commerce外部系统的安全连接。 双向选项需要两个连接：
...
>[!TIP]
>
>请与网络管理员或Cloud Platform提供商合作，以获得有关选择PrivateLink连接类型的帮助，或有关VPC设置和管理方面的帮助。 请参阅Cloud Platform PrivateLink文档： [AWS PrivateLink](https://aws.amazon.com/privatelink/) 或 [Azure专用链接](https://learn.microsoft.com/en-us/azure/private-link/).

## 请求启用PrivateLink

>[!WARNING]
>
>启用PrivateLink最多需要 _五_ 工作日。 提供不完整或不准确的信息可能会延迟该过程。

### 先决条件

![check](../../assets/fix.svg) 云帐户(AWS或Azure)，与Adobe Commerce on cloud infrastructure实例位于同一区域。

![check](../../assets/fix.svg) 客户环境中的VPC，它托管服务以通过PrivateLink进行连接。 请参阅AWS或Azure文档以获取VPC设置帮助或与网络管理员联系。

![check](../../assets/fix.svg) 对于双向PrivateLink连接，在请求PrivateLink启用之前，必须为应用程序或服务创建端点服务配置，并在VPC环境中创建端点。 请参阅 [设置双向PrivateLink连接](#set-up-for-bidirectional-privatelink-connections).
...
- **项目编号** — 提供Adobe Commerce on cloud infrastructure Pro项目ID。 您可以使用以下方式获取项目ID和其他项目信息 [Cloud CLI](../dev-tools/cloud-cli-overview.md) 命令： `magento-cloud project:info`
- **连接类型** — 为连接类型指定单向或双向
- **端点服务** — 对于双向PrivateLink连接，提供Adobe必须连接到的VPC端点服务的DNS URL，例如： `com.amazonaws.vpce.<cloud-region>.vpce-svc-<service-id>`
- **已授予端点服务访问权限** — 要连接到外部服务，请允许端点服务访问以下AWS帐户主体： `arn:aws:iam::402592597372:root`

  >[!WARNING]
  >
  >如果未提供对端点服务的访问，则到VPC中服务的双向PrivateLink连接为 **非** 添加了此标记，这会延迟设置。

#### 特定于Azure专用链接启用的其他先决条件

- 提供群集ID；使用SSH登录到远程数据库并使用命令： `cat /etc/platform_cluster`
...
1. **客户** 提交请求使用主题行启用PrivateLink的支持工单 `PrivateLink support for <company>`. 包括 [启用所需的数据](#prerequisites) 在车票上。 Adobe使用支持工单在启用过程中协调通信。

1. **Adobe** 允许客户帐户访问AdobeVPC中的端点服务。

   - 更新Adobe端点服务配置以接受从客户AWS或Azure帐户启动的请求。
   - 更新支持票证以提供要连接的AdobeVPC端点的服务名称，例如 `com.amazonaws.vpce.<cloud-region>.vpce-svc-<service-id>`.

1. **客户** 将Adobe端点服务添加到其云帐户(AWS或Azure)，这会触发连接请求以Adobe。 有关说明，请参阅云平台文档：

   - 有关AWS的信息，请参阅 [接受和拒绝接口端点连接请求].
   - 有关Azure的信息，请参阅 [管理连接请求].

1. **Adobe** 批准连接请求。

1. 连接请求批准后， **客户** [验证连接](#test-vpc-endpoint-service-connection) VPC和AdobeVPC之间。

1. 启用双向连接的其他步骤：

   - **Adobe** 提供Adobe帐户主体(AWS或Azure帐户的根用户)并请求访问客户VPC端点服务。
   - **客户** 允许Adobe访问客户VPC中的端点服务。 这假设Adobe帐户主体有权访问 `arn:aws:iam::402592597372:root`，如之前所述 **已授予端点服务访问权限** 前提条件。

      - 更新客户端点服务配置以接受从Adobe帐户发起的请求。 有关说明，请参阅云平台文档：

         - 有关AWS的信息，请参阅 [添加和删除端点服务的权限].
         - 有关Azure的信息，请参阅 [管理专用端点连接]

      - 为Adobe提供客户VPC的端点服务名称。

   - **Adobe** 将客户端点服务添加到Adobe平台帐户(AWS或Azure)，这会触发到客户VPC的连接请求。
   - **客户** 批准Adobe的连接请求以完成设置。
   - **客户** [验证连接](#test-vpc-endpoint-service-connection) 从AdobeVPC。
...
1. 从项目根目录中， **签出** 配置为访问PrivateLink端点服务的暂存或生产环境。
...
1. 检查包流。
   ...
   - [AWS：端点服务连接疑难解答]
   - [Amazon：Azure专用链接连接问题疑难解答]

   如果无法解决错误，请更新Adobe Commerce支持票证以请求建立连接的帮助。

## 更改PrivateLink配置

[提交Adobe Commerce支持票证](https://experienceleague.adobe.com/docs/commerce-knowledge-base/kb/help-center-guide/magento-help-center-user-guide.html#submit-ticket) 更改现有的PrivateLink配置。 例如，您可以请求进行如下更改：
...
- An [接口端点] (AWS)或 [专用端点] (Azure)，允许Adobe连接到VPC中托管的端点服务
